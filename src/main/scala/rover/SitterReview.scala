...
object SitterReview {
  ...
  def parseRating(intStr: String, line: List[String]): Try[Int] = Try(intStr.toInt).recoverWith { _ =>
    Failure(new IllegalArgumentException(s"rating '$intStr' was not an integer in row: $line"))
  }
}
